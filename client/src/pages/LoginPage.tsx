import { Lock, ShieldCheck } from "lucide-react";
import { useUser } from "../hooks/useUser";
import { Navigate } from "react-router-dom";
import HelmetSeo from "../components/HelmetSeo";

const loginMetadata = {
  title: "Login - Brainio",
  description:
    "Login to your Brainio account to access personalized learning assessments, track your progress, and take tests.",
  keywords:
    "login, Brainio account, personalized learning, educational tests, sign in",
};

function LoginPage() {
  const handleLogin = () => {
    window.location.href = `${import.meta.env.VITE_SERVER_URL}/auth/google`;
  };

  const { user } = useUser();

  if (user) {
    return <Navigate to="/profile" replace />;
  }

  return (
    <>
      <HelmetSeo
        title={loginMetadata.title}
        description={loginMetadata.description}
        keywords={loginMetadata.keywords}
      />
      <main className="bg-secondary/20 w-full min-h-screen flex justify-center items-center p-6">
        <div className="box box-shadow p-10 flex justify-center items-center flex-col max-w-lg space-y-10">
          <div className="h-fit aspect-square flex justify-center items-center bg-primary/20 p-2 rounded-full">
            <ShieldCheck className="text-primary" size={30} />
          </div>
          <div className="space-y-6">
            <h3 className="text-5xl text-center font-bold tracking-tighter">
              Welcome to Brainio
            </h3>
            <p className="text-center text-lg text-text-muted">
              Sign in to save your progress and view your personalized test
              results.
            </p>
          </div>
          <button
            onClick={handleLogin}
            className="border w-full p-2 outline-none cursor-pointer flex justify-center items-center gap-4 select-none hover:shadow-[5px_5px_var(--color-text)] hover:-translate-[5px] transition duration-300"
          >
            <span>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                x="0px"
                y="0px"
                width="30"
                height="30"
                viewBox="0 0 48 48"
              >
                <path
                  fill="#FFC107"
                  d="M43.611,20.083H42V20H24v8h11.303c-1.649,4.657-6.08,8-11.303,8c-6.627,0-12-5.373-12-12c0-6.627,5.373-12,12-12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C12.955,4,4,12.955,4,24c0,11.045,8.955,20,20,20c11.045,0,20-8.955,20-20C44,22.659,43.862,21.35,43.611,20.083z"
                ></path>
                <path
                  fill="#FF3D00"
                  d="M6.306,14.691l6.571,4.819C14.655,15.108,18.961,12,24,12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C16.318,4,9.656,8.337,6.306,14.691z"
                ></path>
                <path
                  fill="#4CAF50"
                  d="M24,44c5.166,0,9.86-1.977,13.409-5.192l-6.19-5.238C29.211,35.091,26.715,36,24,36c-5.202,0-9.619-3.317-11.283-7.946l-6.522,5.025C9.505,39.556,16.227,44,24,44z"
                ></path>
                <path
                  fill="#1976D2"
                  d="M43.611,20.083H42V20H24v8h11.303c-0.792,2.237-2.231,4.166-4.087,5.571c0.001-0.001,0.002-0.001,0.003-0.002l6.19,5.238C36.971,39.205,44,34,44,24C44,22.659,43.862,21.35,43.611,20.083z"
                ></path>
              </svg>
            </span>
            <span className="text-lg font-semibold">Continue with Google</span>
          </button>
          <div>
            <p className="text-sm text-text-muted flex gap-2 items-center justify-center text-center">
              <Lock size={16} /> Secure Google sign-in. No passwords required.
            </p>
          </div>
        </div>
      </main>
    </>
  );
}

export default LoginPage;
